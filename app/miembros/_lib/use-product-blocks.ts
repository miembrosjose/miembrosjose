...
// Hook useProductBlocks(productId) — blocos de texto editáveis dentro de
// um produto. Mesmo padrão do useEpisodeBlocks.
...
const EVENT = "app:product-blocks-changed"
function emit() {
  if (typeof window !== "undefined") window.dispatchEvent(new CustomEvent(EVENT))
}

export function useProductBlocks(productId: string | null) {
  const [blocks, setBlocks] = useState<DbBlock[]>([])
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const refresh = useCallback(async () => {
    if (!productId) { setBlocks([]); return }
    setLoading(true)
    setError(null)
    try {
      const res = await fetch(`/api/admin/products/${productId}/blocks`, { credentials: "include" })
  ...
  const createBlock = useCallback(
    async (position: BlockPosition, content = "", sortOrder = 0) => {
      ...
      const res = await fetch(`/api/admin/products/${productId}/blocks`, {
        method: "POST",
  ...
  const updateBlock = useCallback(
    async (id: string, patch: Partial<DbBlock>) => {
      const res = await fetch(`/api/admin/product-blocks/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
