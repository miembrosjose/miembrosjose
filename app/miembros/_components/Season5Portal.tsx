...
// ── Journal (bitácora local, autosave en este dispositivo) ───────────
type JournalDef = { key: string; title: string; sub: string; template?: string }

function PortalJournal({ def, onClose }: { def: JournalDef; onClose: () => void }) {
    ...
    <div className={styles.journalOverlay} onClick={(e) => { if (e.target === e.currentTarget) onClose() }}>
      <div className={styles.journalPanel} role="dialog" aria-label={def.title}>
        ...
        <textarea
          ...
          placeholder="Registra aquí comprensiones, sueños, señales, meditaciones, resistencias y cambios interiores…"
        />
...
// ── Fondo cósmico: estrellas + red planetaria + aro de geometría sagrada ──
