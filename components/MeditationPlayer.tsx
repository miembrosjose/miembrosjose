...
// Reproductor de meditaciones — experiencia inmersiva integrada al lenguaje
// visual de Los 144.000 (oscuro, violeta, sobrio, sin estética Spotify).
//
// - Audio servido por /api/meditations/<id>/audio (bucket privado + control de
//   acceso server-side). Nunca recibe el object key ni una URL pública.
// - Play/Pausa prominente, barra interactiva (tocar/arrastrar), ±15s, tiempos.
// - Modo inmersivo: atenúa el entorno y aplica un pulso lento de respiración.
// - Reanuda desde la última posición ("Continuar desde mm:ss") y marca completada
//   al terminar (con tolerancia). Progreso persistido en el servidor.
// - Estado premium BLOQUEADO: mismo lenguaje visual, sin entregar el MP3.
// - Sin autoplay (invasivo + restricciones de navegador). Mobile-first.

import { useCallback, useEffect, useRef, useState, type CSSProperties } from 'react';
import { Play, Pause, RotateCcw, RotateCw, Lock, Sparkles, AlertCircle } from 'lucide-react';
import type { MeditationClient } from '@/app/miembros/_lib/season1-meditaciones';

type Status =
  | 'idle' | 'loading' | 'ready' | 'playing' | 'paused'
  | 'buffering' | 'completed' | 'error';
...
const COMPLETE_TOLERANCE = 1.5; // seg antes del final que cuentan como completado

export default function MeditationPlayer({ meditation }: { meditation: MeditationClient }) {
  const isPremium = meditation.access === 'premium';
  const streamUrl = `/api/meditations/${encodeURIComponent(meditation.id)}/audio`;
  ...
  const startedRef = useRef(false);       // ya se resumió/arrancó una vez
    ...
    try {
      fetch('/api/profile/meditation-progress', {
        ...
        body: JSON.stringify({
          ...
          percent: opts?.completed ? 100 : percent,
          completed: !!opts?.completed,
        }),
      }).catch(() => {});
    } catch { /* ignora */ }
  }, [isPremium, meditation.id]);

  // ── Cargar progreso previo (reanudar / completado) ─────────────────────────
  useEffect(() => {
    if (isPremium) return;
    let cancelled = false;
  ...
  const onTimeUpdate = () => {
    ...
    // Completado con tolerancia cerca del final.
    if (a.duration > 0 && a.currentTime >= a.duration - COMPLETE_TOLERANCE) markCompleted();
    // Guardado periódico.
    const now = Date.now();
    if (!a.paused && now - lastSaveRef.current > SAVE_EVERY_MS) {
  ...
  const onPause = () => {
    ...
    if (startedRef.current && !completedSentRef.current) saveProgress();
  };
  ...
  const togglePlay = () => {
    const a = audioRef.current; if (!a) return;
    if (a.paused) {
      if (!startedRef.current && resumeAt > 0) {
        try { a.currentTime = resumeAt; } catch { /* ignora */ }
      }
  ...
  if (isPremium) {
    return (
      <div className="mp-wrap mp-locked">
        <div className={`mp-cover ${meditation.image ? '' : 'mp-cover--ph'} mp-cover--paid`} style={coverStyle}>
        ...
        <div className="mp-body">
          ...
          {meditation.durationSec ? <p className="mp-metaLine">Duración · {fmt(meditation.durationSec)}</p> : null}
          <div className="mp-lockNote">
            <Sparkles size={15} />
            <span>Esta experiencia requiere acceso adicional.</span>
          </div>
          <button type="button" className="mp-cta mp-cta--gold" disabled title="Disponible pronto">
            {meditation.price ? `Desbloquear · US$ ${meditation.price}` : 'Desbloquear meditación'}
          </button>
          <p className="mp-soon">Disponible pronto</p>
        </div>
        {styleTag}
      </div>
    );
  ...
  return (
    <>
      ...
      <div className={`mp-wrap ${immersive ? 'mp-immersive' : ''}`}>
        ...
        <div className={`mp-cover ${meditation.image ? '' : 'mp-cover--ph'} ${isPlaying ? 'mp-breathe' : ''}`} style={coverStyle}>
          <span className="mp-chip">{completed ? 'Práctica completada' : 'Meditación · Incluida'}</span>
          <span className="mp-coverFade" aria-hidden />
        </div>
        ...
        <div className="mp-body">
          ...
          <h3 className="mp-title">{meditation.title}</h3>
          {meditation.subtitle && !immersive && <p className="mp-sub">{meditation.subtitle}</p>}

          {status === 'error' ? (
            <div className="mp-error">
              <AlertCircle size={16} />
              <span>No se pudo cargar la meditación.</span>
              <button type="button" className="mp-retry" onClick={retry}>Reintentar</button>
            </div>
          ) : (
            <>
              {!startedRef.current && resumeAt > 0 && !completed && (
                <button type="button" className="mp-resume" onClick={togglePlay}>
                  Continuar desde {fmt(resumeAt)}
                </button>
              )}

              <div className="mp-controls">
                <button type="button" className="mp-skip" onClick={() => skip(-15)} aria-label="Retroceder 15 segundos">
                  <RotateCcw size={20} /><span>15</span>
                </button>
                ...
                >
                  {status === 'loading' || status === 'buffering'
                    ? <span className="mp-spin" aria-hidden />
                    : isPlaying ? <Pause size={30} fill="currentColor" /> : <Play size={30} fill="currentColor" />}
                </button>

                <button type="button" className="mp-skip" onClick={() => skip(15)} aria-label="Avanzar 15 segundos">
                  <RotateCw size={20} /><span>15</span>
                </button>
              </div>
              ...
              <div className="mp-progress">
                <input
                  ...
                  onChange={onSeek}
                  style={{ '--mp-pct': `${pct}%` } as CSSProperties}
              ...
              <button
                type="button"
                className={`mp-immersiveBtn ${immersive ? 'is-on' : ''}`}
...
// Estilos (compartidos entre variante bloqueada e incluida).
const styleTag = (
  <style jsx global>{`
    .mp-wrap {
      position: relative; background: #0d0d15; border: 1px solid #1b1c2a;
      border-radius: 14px; overflow: hidden; margin-top: 1.4rem; color: #e8e3d5;
    }
    ...
    .mp-coverFade { position: absolute; inset: 0; pointer-events: none;
    ...
    .mp-chip, .mp-lockChip {
      ...
      letter-spacing: 0.16em; text-transform: uppercase; padding: 0.35rem 0.7rem;
      border-radius: 999px; background: rgba(10,8,18,0.72);
      border: 1px solid rgba(243,246,250,0.14); color: #cbb9e6; backdrop-filter: blur(4px);
    }
    ...
    .mp-kicker {
      ...
      letter-spacing: 0.22em; text-transform: uppercase; color: #6D4A9B; margin: 0 0 0.6rem;
    }
    .mp-kicker--gold { color: #b79554; }
    .mp-title {
      font-family: var(--font-display, serif); font-size: clamp(1.3rem, 4.5vw, 1.6rem);
      line-height: 1.15; color: #f3f0e8; margin: 0 0 0.6rem;
    }
    .mp-sub { font-size: 0.98rem; line-height: 1.55; color: #a9a397; margin: 0 0 1.2rem; }
    .mp-metaLine { font-family: var(--font-mono, monospace); font-size: 0.7rem; letter-spacing: 0.12em; color: #7c8088; margin: 0 0 1rem; }
    ...
    .mp-play {
      ...
      background: radial-gradient(circle at 50% 35%, #7c53ad, #4A3170);
      color: #f3f0e8; border: 1px solid #8a63b8;
      box-shadow: 0 0 0 6px rgba(109,74,155,0.12), 0 10px 30px rgba(74,49,112,0.45);
    ...
    .mp-skip {
      ...
      background: none; border: none; cursor: pointer; color: #b9b3c6;
      font-family: var(--font-mono, monospace); font-size: 0.58rem; letter-spacing: 0.05em;
    ...
    /* Barra de progreso (nativa, estilizada — táctil en móvil) */
    ...
    .mp-range::-webkit-slider-runnable-track {
      ...
      background: linear-gradient(90deg, #8a63b8 0%, #8a63b8 var(--mp-pct, 0%), rgba(243,246,250,0.12) var(--mp-pct, 0%), rgba(243,246,250,0.12) 100%);
    }
    ...
    .mp-range::-webkit-slider-thumb {
      ...
      width: 18px; height: 18px; border-radius: 50%; background: #f3f0e8;
      border: 3px solid #6D4A9B; box-shadow: 0 0 10px rgba(138,99,184,0.6);
    }
    ...
    .mp-times {
      ...
      font-family: var(--font-mono, monospace); font-size: 0.72rem; color: #8a8598;
    }

    .mp-resume {
      display: block; width: 100%; margin-bottom: 1rem; padding: 0.7rem 1rem;
      font-family: var(--font-mono, monospace); font-size: 0.72rem; letter-spacing: 0.1em;
      color: #cbb9e6; background: rgba(109,74,155,0.1); border: 1px solid #4A3170;
      border-radius: 8px; cursor: pointer; transition: background 0.2s ease;
    }
    .mp-resume:hover { background: rgba(109,74,155,0.2); }

    .mp-immersiveBtn {
      display: block; margin: 1.2rem auto 0; padding: 0.5rem 1rem;
      font-family: var(--font-mono, monospace); font-size: 0.62rem; letter-spacing: 0.18em;
      text-transform: uppercase; color: #7c8088; background: none;
      border: 1px solid #1b1c2a; border-radius: 999px; cursor: pointer; transition: all 0.2s ease;
    }
    .mp-immersiveBtn:hover, .mp-immersiveBtn.is-on { color: #cbb9e6; border-color: #4A3170; }

    /* Premium bloqueado */
    .mp-lockNote {
      display: flex; align-items: center; gap: 0.5rem; margin: 0.4rem 0 1.2rem;
      color: #b9b3a6; font-size: 0.95rem;
    }
    .mp-lockNote svg { color: #b79554; flex-shrink: 0; }
    .mp-cta {
      display: block; width: 100%; text-align: center; cursor: pointer;
      font-family: var(--font-mono, monospace); font-size: 0.74rem; letter-spacing: 0.2em;
      text-transform: uppercase; padding: 1rem 1.2rem; border-radius: 10px; border: 1px solid transparent;
    }
    .mp-cta--gold { background: linear-gradient(180deg, #d9b866 0%, #c39f4e 100%); color: #221a08; font-weight: 700; border-color: #b79554; opacity: 0.55; cursor: not-allowed; }
    .mp-soon { text-align: center; font-family: var(--font-mono, monospace); font-size: 0.6rem; letter-spacing: 0.15em; text-transform: uppercase; color: #6a6a85; margin: 0.7rem 0 0; }
    ...
    @media (max-width: 480px) {
      .mp-controls { gap: 1.15rem; }
      .mp-play { width: 66px; height: 66px; }
    }
    @media (prefers-reduced-motion: reduce) {
      .mp-breathe { animation: none; }
    }
  `}</style>
);
